...
export function setCookie(name, value, seconds) {
  ...
  // 转码并赋值
  document.cookie = name + '=' + escape(value) + expires + '; path=/marubi';
}

/** --------- 数据字典相关(先存储后获取,保证获取的时候必须有数据) ----------- */
// 存储对应数据字典列表
export const setDictList = (type) => {
  // 如果sessionStorage中已经存在对应数据则直接返回
  const list = getSession(type);
  if (list) {
  ...
  // 否则发请求之后获取数据
  const params = { type };
  return dictionaries.getValueList(params).then(res => {
    let dictList = [];
    if (res.data.code === 0) {
      const data = res.data.data;

      dictList = data.map(item => ({ value: item.value, label: item.name, description: item.description }));
    }
    setSession(type, dictList);
...
// 获取对应数据字典列表
export const getDictList = (type) => {
  const dictList = getSession(type);
